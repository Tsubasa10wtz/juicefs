 ...
 */
 ...
 func newFileSystem(conf *vfs.Config, v *vfs.VFS) *fileSystem {
	 ...
	 fs.t = NewStreamTree(fs) // 传递fs指针
	 // go fs.startHttpListener()
	 return fs
 }
 
 type setTimeout func(time.Duration)
 
 //func (fs *fileSystem) ProcessBaseMsg(w http.ResponseWriter, body []byte) {
 //	// 先解析基本消息
 ...
 //	// 根据 Type 解析为具体的消息类型
 //	switch baseMsg.Type {
 //	case 1: // 假设 1 表示 LookupMessage
 //		var msg LookupMessage
 //		if err := json.Unmarshal(body, &msg); err != nil {
 //			http.Error(w, "Error parsing LookupMessage", http.StatusBadRequest)
 //		} else {
 //			// 处理 OpenMessage
 //			go fs.t.ProcessLookup(msg.Ino, msg.NameIno, msg.Name, msg.Timestamp)
 //		}
 //	case 2: // 假设 2 表示 OpenMessage
 //		var msg OpenMessage
 //		if err := json.Unmarshal(body, &msg); err != nil {
 //			http.Error(w, "Error parsing OpenMessage", http.StatusBadRequest)
 //		} else {
 //			// 处理 OpenMessage
 //			go fs.t.ProcessOpen(msg.Ino, msg.Timestamp)
 //		}
 //
 //	case 3: // 假设 3 表示 ReadMessage
 //		var msg ReadMessage
 //		if err := json.Unmarshal(body, &msg); err != nil {
 //			http.Error(w, "Error parsing ReadMessage", http.StatusBadRequest)
 //		} else {
 //			// 处理 ReadMessage
 //			go fs.t.ProcessRead(msg.Ino, msg.Offset, msg.Size, msg.Timestamp)
 //		}
 //
 //	case 4: // 假设 3 表示 ReadDirMessage
 //		var msg ReadMessage
 //		if err := json.Unmarshal(body, &msg); err != nil {
 //			http.Error(w, "Error parsing ReadMessage", http.StatusBadRequest)
 //		} else {
 ...
 //func (fs *fileSystem) startHttpListener() {
 ...
 func (fs *fileSystem) OpenDirForNames(cancel <-chan struct{}, in *fuse.OpenIn, out *fuse.OpenOut) (status fuse.Status) {
	 ctx := fs.newContext(cancel, &in.InHeader)
	 defer releaseContext(ctx)
	 fh, err := fs.v.Opendir(ctx, Ino(in.NodeId), in.Flags)
 ...
 func (fs *fileSystem) ReadDirNames(cancel <-chan struct{}, in *fuse.ReadIn) ([]string, fuse.Status) {
	 ctx := fs.newContext(cancel, &in.InHeader)
	 defer releaseContext(ctx)
 
	 entries, _, err := fs.v.Readdir(ctx, Ino(in.NodeId), in.Size, int(in.Offset), in.Fh, true)
	 if !errors.Is(err, syscall.Errno(0)) {
		 return nil, fuse.Status(err)
	 }
